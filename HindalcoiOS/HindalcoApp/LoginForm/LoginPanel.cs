...
namespace RMPCLApp.Login_Form
{
    public partial class LoginPanel : XtraForm
    {
        ...
        {
            InitializeComponent();
            this.lblVersion.Text = "@Copyright 2021 IndustryOS";
        }
        ...
        private void UpdateConfig(string key, string value, string serverName,string UserName,String Password)
        {
            ...
            {
                Value= "Data Source="+serverName+";Initial Catalog="+value+";user id=sa;password=IndustryOs!!20@21";
            }
            else
            {
                Value = "Data Source=" + serverName + ";Initial Catalog=" + value + ";user id="+UserName+";password="+ Password+"";
            }
        ...
        private void btnlogin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Result == "Logged In Successfully!")//Logged In Successfully
                {
                    ...
                    int RoleId = 0;
                    Resources.Instance.GetMaxID("GetUserGuid", "@RoleID", UserID, ref RoleId);
                    RMPCLApp.Properties.Settings.Default.RoleID = RoleId;
                    //Store User Name temporary
                    RMPCLApp.Properties.Settings.Default.UserLog = UserName;
                    RMPCLApp.Properties.Settings.Default.Save();
                    //Store Email Id temporary
                    RMPCLApp.Properties.Settings.Default.Email = txtEmail.Text;
                    RMPCLApp.Properties.Settings.Default.Save();
                    // Store GeoLocation temporary
                    RMPCLApp.Properties.Settings.Default.GeoLocation = GeoLocation;
                    RMPCLApp.Properties.Settings.Default.Save();
                    ...
                    if (GlobalDeclaration.FirstLogOut)
                    {
                        ...
                        if (((RMPCLApp.ParentWindow)form).OnAfterLogOut != null)
                            ((RMPCLApp.ParentWindow)form).OnAfterLogOut.Invoke(sender, this.Name);
                        GlobalDeclaration.FirstLogOut = false;
                        this.Close();
                        form.Show();
                        ((RMPCLApp.ParentWindow)form).OnAfterLogOut -= ((RMPCLApp.ParentWindow)form).OnAfterLogOutReCall;

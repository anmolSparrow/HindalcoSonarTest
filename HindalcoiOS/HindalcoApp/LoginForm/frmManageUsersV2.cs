...
namespace HindalcoiOS.Login_Form
{
    public partial class frmManageUsers : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            dgvUserdata.DataSource = Resources.Instance.GetDataKey("SP_SearchViewUsers", "p_Search", txtSearch.Text);
            dgvUserdata.Refresh();
        }
        private void dgvUserdata_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    temp = dgvUserdata.SelectedRows[0].Cells["ID"].Value.ToString();

...
{
    public partial class AuditTracker : DevExpress.XtraEditors.XtraForm
    {
        public AuditTracker()
        {
            InitializeComponent();
        ...
        private void AuditTracker_Load(object sender, EventArgs e)
        {
            List<Control> allControls = GetAllControls(this);
            allControls.ForEach(k => k.Font = new System.Drawing.Font("Segoe UI", 10));
            usercontrol = new TrackerControl("Internal");
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string value = code + "_" + authername + "_" + namofau + "_" + "Internal" + "_" + date;
                if (DgvStatus.Rows.Count > 0)
                {
                    DgvStatus.Rows.Clear();
                }
                DataTable Dt = Resources.Instance.GetDataKey("SP_GetAuditCategory", "@auditcode", "@auditorname", "@auditname", "@action", "@date", value, 2);
                if (Dt.Rows.Count > 0)
                {

...
{
    public partial class Machine_Edit_Data2 : XtraForm
    {
        ...
        public Machine_Edit_Data2(string machinename, string SysGenMachNo)
        {
            InitializeComponent();
        ...
        private void AddIneventoryHeader()
        {
            ...
            var mtgno = addinventory.Columns.Add("Machine Tag No");
            mtgno.ColumnName = "Machine TagNo";
            
            var itmmn=addinventory.Columns.Add("Machine Name");
            itmmn.ColumnName="Item Name";
            addinventory.Columns.Add("Stock In Hand");
            addinventory.Columns.Add("Unit");
            addinventory.Columns.Add("Entry Date");
            addinventory.Columns.Add("Model No.");
            addinventory.Columns.Add("Minimum Stock");
            addinventory.Columns.Add("OEM");
            addinventory.Columns.Add("Last Ordered Date");
            addinventory.Columns.Add("Minimum Lead Time");
        ...
        //    {
        //        AddIneventoryHeader();
        //        addinventory = Resources.Instance.getInventoryData(this.MachinType,this.MachineSybType,this.machineName);
        ...
        private void btnupdat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GlobalDeclaration.UserType.Equals("Admin") || GlobalDeclaration.UserType.Equals("U0-User"))
                {
                    ...
                    {
                        string valueslist = this.MachineName + "~" + txttagno.Text + "~" + this.SysGenMachineNo + "~" +
                        ...
                        DataRow[] Update = Resources.Instance.MachineDt.Select("MachineTagNo='" + txttagno.Text + "' and MachineName='" + this.MachineName + "' and CadLocation='" + this.cmblocation.SelectedItem.ToString() + "'");
        ...
        private void btnPriceTenstive_Click(object sender, EventArgs e)
        {
            try
            {
                Tentive_Price T_price = new Tentive_Price(this.SysGenMachineNo, this.MachineName, this.txttaggedMachine.Text);
        ...
        private void UpdateInventoryHeader()
        {
            ...
            updinventory.Columns.Add("Machine Tag No");
            updinventory.Columns.Add("Machine Name");
            updinventory.Columns.Add("Stock In hand");
            updinventory.Columns.Add("Unit");
            updinventory.Columns.Add("Entry Date");
            updinventory.Columns.Add("Model No.");
            updinventory.Columns.Add("Minimum Stock");
            updinventory.Columns.Add("OEM");
            updinventory.Columns.Add("Last Ordered Date");
            updinventory.Columns.Add("Minimum Lead Time");
        ...
        //            dataInvRow[10] = this.MachinType;
        //            dataInvRow[11] = this.MachineSybType;
        ...
        //        {
        //            DataTable getdata = new DataTable();
        //            getdata = Resources.Instance.getInventoryAllData();
        ...
        {
            string tage = ((BunifuButton)sender).Tag.ToString();
                ...
                case "SpecificationTab":
                    {
                       ...
                       // RemoveTabPages();
                        ...
                        {
                           //currentTab =SpecificationPages.SelectedIndex;
                    ...
                    if (DgvResult.Rows.Count > 0)
                    {
                        if (DgvResult.SelectedRows.Count > 0)
                        {
                            if (DgvResult.SelectedRows[0].Cells["Mtag"] != null)
                            {
                                ...
                                ResultRecordCount = DgvResult.Rows.Count;
                        ...
                        {
                            XtraMessageBox.Show(new Form { TopMost = true }, "Please select Row First.Then Make Duplicate Entry against Selected Row?", ApplicationConstants.MessageDisplay, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                    }
                    else
                    {
                        XtraMessageBox.Show(new Form { TopMost = true }, "Please Generate Row First From Basic Details Tab Then Create Duplicate Row Against Selected Machine Tag?", ApplicationConstants.MessageDisplay, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
                else
                {
                    try
                    {
                        if (DgvResult.SelectedRows.Count > 0)
                        {
                            ...
                            {
                                DgvResult.Rows.RemoveAt(DgvResult.SelectedRows[0].Index);
                                ResultRecordCount = DgvResult.RowCount;
                            }
                        }
                    }
                    catch (Exception Ex)
            ...
            try
            {
                if (DgvMaitenance.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < looprn; i++)//DgvMaitenance.Rows[i].Cells["Team"].Value.ToString()
                        {
                            query = query + "(" + DgvMaitenance.Rows[i].Cells["SrNo"].Value + ",'" + this.txttagno.Text + "','" + this.MachineName + "','"
                                + DgvMaitenance.Rows[i].Cells["Activity"].Value.ToString() + "','" + DgvMaitenance.Rows[i].Cells["Team"].Value.ToString() + "' ,'"
                                + DgvMaitenance.Rows[i].Cells["Dlast"].Value.ToString() + "','" + DgvMaitenance.Rows[i].Cells["Mdate"].Value.ToString() + "','"
                ...
                if (DgvResult.Rows.Count > 0)
                {
                    ...
                    if (ReceiveCount > 0)
                    {
                        if (DgvResult.Rows.Count > 0)
                        {
                            looprn = DgvResult.Rows.Count - ResultRecordCount;//objactivity                           
                            for (int i = DgvResult.Rows.Count - 1; i >= ResultRecordCount; i--)
                            {
                    ...
                    if (!_SheetName.Contains(sheetName))
                    {
                        ...
                        if (DgvMaitenance.Columns.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dataset.Tables[sheetName].Rows.Count; i++)
                            {
                                ...
                                DgvMaitenance.Rows[i].Cells["Activity"].Value = row["Activity Detail"].ToString();
        ...
        {
            LogDatacs _logdata = new LogDatacs(txttaggedMachine.Text, this.MachineName, this.SysGenMachineNo, this.cmblocation.SelectedItem.ToString());
        ...
        {
            JSAInput _JSAInput = new JSAInput(this.txttaggedMachine.Text);
        ...
        BunifuDataGridView dbview = new BunifuDataGridView();
         //dbview.AllowCustomTheming = true;
        ...
        private void btnUploadFiles_Click(object sender, EventArgs e)
        {
            try
            {
                if (txttagno.Text != "")
                {
                    string desiredPath = Application.StartupPath + @"\MCD_FileUploads" + @"\" + txttaggedMachine.Text;
        ...
        private void txtMachineAutoSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string[] postSource = getDTS
                            .AsEnumerable()
                            .Select<System.Data.DataRow, String>(x => x.Field<String>("Machine Name"))
                            .ToArray();
        ...
        //private void AddTaggedMachine()
        //{
            
        //    //var mtgno = addinventory.Columns.Add("Machine Tag No");
        //    //mtgno.ColumnName = "Machine TagNo";
        //    TaggedMAchine = new DataTable();
        //    TaggedMAchine.Columns.Clear();
        //    TaggedMAchine.Rows.Clear();
        //    var itmmn = TaggedMAchine.Columns.Add("Machine Name");
        //    itmmn.ColumnName = "Item Name";
        //    TaggedMAchine.Columns.Add("Machine Tag No");
        //}
        ...
        private void txtMachineAutoSearch_TextChanged_1(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnTaggInvt_Click_1(object sender, EventArgs e)
        {
            try
            {
                int aid = 0;
